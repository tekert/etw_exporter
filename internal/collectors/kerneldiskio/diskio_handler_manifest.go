...
func NewDiskIOHandlerManifest(config *config.DiskIOConfig, sm *statemanager.KernelStateManager) *HandlerManifest {
	return &HandlerManifest{
		customCollector: nil,
		config:          config,
		stateManager:    sm,
		log:             logger.NewSampledLoggerCtx("diskio_handler_manifest"),
	}
}

// AttachCollector allows a metrics collector to subscribe to the handler's events.
func (c *HandlerManifest) AttachCollector(collector *DiskCollector) {
...
func (h *HandlerManifest) RegisterRoutes(router handlers.Router) {
	...
	// TODO: move this to fiel module, not needed for now.
	if false {
...
func (d *HandlerManifest) HandleDiskRead(helper *etw.EventRecordHelper) error {
...
//
// from Kernel-File events (Create, Rundown) and looking up the FileObject from this event.
func (d *HandlerManifest) HandleDiskWrite(helper *etw.EventRecordHelper) error {
	if d.customCollector == nil {
		return nil
	}
...
func (d *HandlerManifest) HandleDiskFlush(helper *etw.EventRecordHelper) error {
	if d.customCollector == nil {
		return nil
	}
...
func (d *HandlerManifest) HandleFileCreate(helper *etw.EventRecordHelper) error {
	return nil
}
...
func (d *HandlerManifest) HandleFileClose(helper *etw.EventRecordHelper) error {
	return nil
}
...
func (d *HandlerManifest) HandleFileWrite(helper *etw.EventRecordHelper) error {
	return nil
}
...
// This handler ensures the FileObject-to-ProcessID mapping is kept up-to-date, as
// a file handle might be used by a process long after it was created.
func (d *HandlerManifest) HandleFileRead(helper *etw.EventRecordHelper) error {
	return nil
}
...
func (d *HandlerManifest) HandleFileDelete(helper *etw.EventRecordHelper) error {
	return nil
}
